// Our special "ng serve" substitute that serves all applications at the same time.
...
function buildProject(task, completedCallback) {
  ...
  const projectNameWithoutNumber = projectName.replace(
    `${prefix}-`,
    ''
  );
  const command = [
    ...
    'build',
    projectName,
    `--baseHref=/${projectNameWithoutNumber}/`,
