...
export class BlogModule {
  constructor(store: Store, userLoaderService: UserLoaderService) {
    // todo: This is not the standard way to do this; we'll see a
    // better way with Effects. One problem is that there's no way to
    // unsubscribe from this manual subscription because there are no
    // lifecycle hooks on modules.
    userLoaderService.load().subscribe(users => {
      store.dispatch(userApiActions.loadUsersSuccess({ users }));
    });
  }
}
